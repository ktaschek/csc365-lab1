...
def query_student(students: list, last_name: str, bus: bool = False):
  for student in students:
    if student.st_last_name == last_name:
      if bus:
        print(f"{student.st_last_name},{student.st_first_name},{student.bus}")
      else:
...
def query_grade(students: list, grade: str, high: bool = False, low: bool = False):
  if high:
    student = max((student for student in students if student.grade == grade), key=lambda student: student.gpa)
    print(f"{student.st_last_name},{student.st_first_name},{student.gpa},{student.t_last_name},{student.t_first_name},{student.bus}")
  elif low:
    student = min((student for student in students if student.grade == grade), key=lambda student: student.gpa)
    print(f"{student.st_last_name},{student.st_first_name},{student.gpa},{student.t_last_name},{student.t_first_name},{student.bus}")
  else:
    for student in students:
...
def query_bus(students: list, bus: str):
  for student in students:
    if student.bus == bus:
        print(f"{student.st_last_name},{student.st_first_name},{student.grade},{student.classroom}")

def query_average(students: list, grade: str):
  students_in_grade = [student for student in students if student.grade == grade]
  average = sum(student.gpa for student in students_in_grade) / len(students_in_grade)
  print(f"{grade},{average}")

def query_info(students: list):
  students_by_grade = [[student for student in students if student.grade == grade] for grade in range(GRADES)]
...
def parse_query(students: list, query: str):
  ...
  if len(tokens) >= 2 and tokens[0] in ["S", "Student"]:
    ...
    if len(tokens) >= 3 and tokens[2] in ["H", "High"]:
      query_grade(students, tokens[1], high=True)
    elif len(tokens) >= 3 and tokens[2] in ["L", "Low"]:
